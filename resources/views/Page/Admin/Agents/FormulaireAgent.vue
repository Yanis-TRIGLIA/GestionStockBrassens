            ...
            <div class="bg-white shadow-lg rounded-lg p-6">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-2">
                        <h2 class="text-xl font-semibold mb-4">Informations agent</h2>
                    ...
                    <div>
                        ...
                        <div 
                            ...
                            <div v-if="imagePreview" class="mt-4">
                                <img :src="fullImageUrl" alt="Aperçu" class="w-full h-40 object-cover rounded-md shadow-sm" />
                                <div class="flex justify-between mt-2">
                                    <button type="button" @click="removeImage" class="px-3 py-2 bg-red-500 text-white rounded-md text-sm">Retirer</button>
                                    <button type="button" @click="openInNewTab" class="px-3 py-2 bg-gray-100 rounded-md text-sm">Ouvrir</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-6 flex items-center justify-end gap-3">
                    <button type="button" @click="$emit('cancel')" class="px-4 py-2 rounded-md bg-gray-100">Annuler</button>
...
export default {
    ...
    data() {
        return {
            formData: {
                nom: this.agent?.nom || "",
                type: this.agent?.type || "personne",
                image: null,
            },
            baseUrl: import.meta.env.VITE_APP_URL,
            imagePreview: this.agent?.image_url || null,
            isDragOver: false,
            dragCounter: 0,
            showCropModal: false,
            tempImageSrc: null,
    ...
    watch: {
        agent: {
            ...
            handler(newAgent) {
                if (newAgent) {
                    this.formData.nom = newAgent.nom || "";
                    this.formData.type = newAgent.type || "personne";
                    if (newAgent.image_url && !this.imagePreview) {
                        this.imagePreview = newAgent.image_url;
                    }
                }
            }
        },
    ...
    methods: {
                ...
                const initializeCropper = () => {
                    ...
                    try {
                        this.cropper = new CropperClass(img, {
                            // Aspect ratio de la zone (rognage libre)
        ...
        submitForm() {
            ...
            // 1. Émettre l'événement pour la sauvegarde (dans le parent)
            this.$emit("submit", formData);

            // 2. 🚀 Redirection vers /admin/agent
            if (this.$router) {
                this.$router.push('/admin/agent');
            } else {
                console.error("Vue Router non trouvé. Impossible de rediriger.");
            }
        },
    },
